import type { EditorBlock, EditorBlockLayout } from "@chulane/rivto";
...
export function rootOwnerId(blocks: readonly EditorBlock[], blockId: string): string | undefined {
  const contains = (block: EditorBlock): boolean => (
...
export function owningRootIds(blocks: readonly EditorBlock[], blockIds: readonly string[]): string[] {
  const requested = new Set(blockIds);
  const containsRequested = (block: EditorBlock): boolean => (
...
export function translatedLayouts(
  ...
  dx: number,
  dy: number,
): Array<{ id: string; layout: Pick<EditorBlockLayout, "x" | "y"> }> {
