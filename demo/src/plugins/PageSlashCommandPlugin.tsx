...
import { COUNTER_BLOCK_TYPE } from "../blocks/custom-blocks";
...
/**
 * Demo-owned inline slash menu for editable page blocks.
 *
...
export function PageSlashCommandPlugin() {
  ...
  useEditorEvent("input", (event) => {
    const content = event.target instanceof Element
      ...
      : null;
    const blockId = content?.closest<HTMLElement>(BLOCK_ID_SELECTOR)?.dataset.blockId;
    if (!content || !blockId || !root?.contains(content)) return;
    // Counter is contentless; Markdown and Slider both expose the same marker.
    if (editor.getBlock(blockId)?.type === COUNTER_BLOCK_TYPE) return;
