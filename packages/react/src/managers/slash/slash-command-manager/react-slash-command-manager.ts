 ...
 * caret geometry, and keyboard navigation remain presentation-plugin concerns.
 */
export class ReactSlashCommandManager {
  ...
  /**
   * Registers a core command owned by the React runtime and active plugin.
   *
   * @param command - Complete command stored by the core slash manager.
   * @returns Idempotent lifecycle-owned command disposer.
   */
  register(command: SlashCommand): () => void {
    const { editor, plugins } = this.reactEditor;
    plugins.assertActive();
