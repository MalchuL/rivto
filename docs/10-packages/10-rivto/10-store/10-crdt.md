...
Пакет `packages/crdt-doc` (`@chulane/crdt-doc`) — независимая от UI граница Rivto для совместно редактируемых данных. Остальная часть core-пакета зависит от небольших контрактов `CRDTDoc`, `CRDTMap`, `CRDTArray` и `CRDTText`, а не импортирует Yjs напрямую. `@chulane/rivto` реэкспортирует этот публичный API.

Сейчас эти контракты реализованы через [Yjs](https://yjs.dev/). Импорты нативного Yjs остаются внутри `packages/crdt-doc/src/yjs-doc`. Благодаря этому модель документа проще тестировать, а в будущем можно добавить другую CRDT-реализацию.
