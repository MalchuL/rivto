...
test("renders every root as one card with its complete nested outline", async ({ page }) => {
  const pageRoots = page.locator(".page-surface > [data-block-id]");
  ...
  const childId = await parent.locator(":scope > .page-block-children [data-block-id]").first().getAttribute("data-block-id");
  ...
  await expect(card.locator(`[data-block-id="${childId}"]`)).toHaveCount(1);
  ...
  await expect(page.locator(`[data-block-id="${parentId}"] > .page-block-children`)).toHaveCount(0);
});
...
test("reuses Tab and Shift+Tab inside a canvas card", async ({ page }) => {
  ...
  const firstId = await directChildren.first().getAttribute("data-block-id");
  ...
  const first = rootBlock.locator(`.page-block[data-block-id="${firstId}"]`);
  await expect(first.locator(`:scope > .page-block-children > [data-block-id="${secondId}"]`)).toHaveCount(1);

  await first.locator(`[data-block-id="${secondId}"] > .page-block-row [data-block-content]`).click();
  await page.keyboard.press("Shift+Tab");
  await expect(rootBlock.locator(`:scope > .page-block-children > [data-block-id="${secondId}"]`)).toHaveCount(1);
});
...
test("reuses page Enter and structural drag inside a canvas card", async ({ page }) => {
  ...
  await page.mouse.up();
  const movedTarget = rootBlock.locator(`.page-block[data-block-id="${targetId}"]`);
  await expect(movedTarget.locator(`:scope > .page-block-children > [data-block-id="${sourceId}"]`)).toHaveCount(1);
});
...
test("edits Markdown and custom controls without selecting their root cards", async ({ page }) => {
  ...
  const slider = page.locator('[data-edgeless-root][data-block-type="demo.slider"] input');
  await slider.focus();
  await page.keyboard.press("ArrowRight");
  await expect(slider).toHaveValue("36");
  const counter = page.locator('[data-edgeless-root][data-block-type="demo.counter"] .custom-counter-block');
  await counter.click();
...
test("toggles root selection and moves or resizes layouts atomically", async ({ page }) => {
  ...
  await expect(page.locator("[data-edgeless-root][data-selected]")).toHaveCount(2);
...
test("zooms, pans, and pastes selected root subtrees with offset layouts", async ({ page }) => {
  ...
  await expect(page.locator("[data-edgeless-root][data-selected]")).toHaveCount(1);
...
test("duplicates a complete root subtree from slash with offset geometry", async ({ page }) => {
  ...
  const originalDescendants = await original.locator("[data-block-id]").count();
  const before = await cards.count();
  ...
  const duplicate = page.locator("[data-edgeless-root][data-selected]");
  await expect(duplicate).toHaveCount(1);
  await expect(duplicate).not.toHaveAttribute("data-edgeless-root", originalId ?? "");
  await expect(duplicate.locator("[data-block-id]")).toHaveCount(originalDescendants);
