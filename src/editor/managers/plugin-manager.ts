...
export interface RivtoPlugin {
  ...
  /** Additional slash actions not generated by a block definition. */
...
export class PluginManager {
   ...
   *
   * Plugin-level modes become defaults for block definitions and UI items, but
   * an individual contribution may narrow or override those modes. Commands
  ...
  use(plugin: RivtoPlugin): () => void {
    ...
    try {
      plugin.blocks?.forEach((block) => remove.push(this.blocks.register({
        ...block,
        supportedModes: block.supportedModes ?? plugin.modes,
      })));
      for (const [name, handler] of Object.entries(plugin.commands ?? {})) {
        const command = this.commands.registerDynamic(name, (payload: unknown) => {
